...
_MEMORY_REVIEW_PROMPT = (
    ...
    "'Nothing to save.' is the correct default — most sessions reveal nothing "
    "durable. Save only:\n"
    "1. Who the user is: persona, situation, personal details worth remembering.\n"
    "2. Durable expectations about how you should behave or how they work.\n\n"
    "Do not save anything you cannot point to in the conversation above, and do "
    "not save what SOUL.md, the configuration, or an existing memory already "
    "covers.\n\n"
    "If something clears that bar, save it with the memory tool. Otherwise reply "
    "exactly 'Nothing to save.' and stop."
)

_SKILL_REVIEW_PROMPT = (
    "Review the conversation above and decide whether the skill library should "
    "change.\n\n"
    "**'Nothing to save.' is the correct default.** Most sessions produce no "
    "durable skill change. Say it and stop unless a rule below clearly fires. "
    "A marginal proposal costs the user a review; proposing nothing costs "
    "nothing.\n\n"
    "**Evidence rule.** You are replaying a transcript. You have no terminal, "
    "no file access and no database — only the conversation above and the "
    "skills you read with skill_view during this pass. Any claim about a "
    "database, schema, table, script, file or command must be supported by "
    "something you can point to in one of those two places. If you cannot, "
    "propose nothing: do not infer it from names, reconstruct it, or write it "
    "down as probably true.\n\n"
    "**Already-covered rule.** If SOUL.md, the agent's configuration, or an "
    "existing skill already covers it, it is not a finding. Restating existing "
    "guidance somewhere new is a duplicate, not a learning.\n\n"
    "Two things clear the bar. A correction the user actually made this "
    "session — style, tone, format, verbosity, legibility, workflow or "
    "approach — is a FIRST-CLASS skill signal, not just a memory one ('stop "
    "doing X', 'don't format like this', an explicit 'remember this'); embed "
    "it in the skill that governs that task. So is a non-trivial technique, "
    "fix or debugging path that demonstrably worked here. Nothing else does.\n\n"
    "Target shape: CLASS-LEVEL skills with a rich SKILL.md and a `references/` "
    "directory, not a flat list of one-session entries.\n\n"
    "Preference order — earliest that fits:\n"
    "  1. PATCH a skill loaded via /skill-name or read with skill_view this "
    "session, if it covers the ground.\n"
    "  2. PATCH an existing class-level umbrella (skills_list + skill_view).\n"
    "  3. ADD a support file under an existing umbrella via skill_manage "
    "action=write_file: `references/<topic>.md` for detail and condensed "
    "knowledge (quoted research, API excerpts, domain notes); "
    "`templates/<name>.<ext>` for starter files meant to be copied; "
    "`scripts/<name>.<ext>` for re-runnable actions. Add a one-line pointer in "
    "SKILL.md.\n"
    "  4. CREATE a new class-level umbrella only when nothing above fits. The "
    "name must be class-level — never a PR number, error string, codename, "
    "library-alone name, or 'fix-X / debug-Y' session artifact. If the name "
    "only makes sense for today's task, go back to 1-3.\n\n"
    "Protected skills — your writes WILL be refused: bundled, hub-installed, "
    "skills.external_dirs, PINNED, and any user-owned skill, including ones "
    "loaded this session. Being in play does not make one yours. If such a "
    "skill is wrong, say so and recommend 'hermes curator adopt <name>'. If "
    "only protected skills need updating, say 'Nothing to save.' and stop.\n\n"
    "Do NOT capture — these harden into constraints that outlive the problem:\n"
    "  • Environment-dependent failures: missing binaries, 'command not "
    "found', uninstalled packages, unconfigured credentials, fresh-install or "
    "post-migration path errors. Capture the FIX under an existing setup or "
    "troubleshooting skill, never the failure.\n"
    "  • Negative claims: 'X tool is broken', 'browser tools do not work'. "
    "The agent cites these against itself for months after the fix.\n"
    "  • Transient errors that resolved before the session ended. If a retry "
    "worked, the lesson is the retry.\n"
    "  • One-off task narratives ('summarize today's market', 'analyze this "
    "PR').\n"
    "  • Unresolved failures: if the session ended without a working method "
    "— you tried several things, none worked, and told the user to check "
    "manually — do NOT write those attempts up as a reliable workflow or "
    "recommended approach. Say 'Nothing to save.' Capture an alternative only "
    "if you are independently confident it works, and never the dead ends.\n\n"
    "If two existing skills overlap, mention it in your reply — the curator "
    "handles consolidation.\n\n"
    "If nothing clears the bar, reply exactly 'Nothing to save.'"
)

_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and decide whether memory or the skill "
    "library should change.\n\n"
    "**'Nothing to save.' is the correct default.** Most sessions produce "
    "nothing durable on either dimension. Say it and stop unless a rule below "
    "clearly fires. A marginal proposal costs the user a review; proposing "
    ...
    "down as probably true.\n\n"
    "**Already-covered rule.** If SOUL.md, the agent's configuration, or an "
    "existing memory or skill already covers it, it is not a finding. "
    "Restating existing guidance somewhere new is a duplicate, not a "
    "learning.\n\n"
    "**Memory** — who the user is: persona, durable preferences, personal "
    "details, expectations about how you should behave. Save those with the "
    "memory tool.\n\n"
    "**Skills** — how to do this class of task. Two things clear the bar. A "
    "correction the user actually made this session — style, tone, format, "
    "verbosity, legibility, workflow or approach — is a FIRST-CLASS skill "
    "signal, not just a memory one; embed it in the skill that governs that "
    "task so the next session starts fixed. So is a non-trivial technique, fix "
    "or debugging path that demonstrably worked here. Nothing else does.\n\n"
    "Target shape: CLASS-LEVEL skills with a rich SKILL.md and a `references/` "
    "directory, not a flat list of one-session entries.\n\n"
    ...
    "skill is wrong, say so and recommend 'hermes curator adopt <name>'. If "
    "only protected skills need updating, say 'Nothing to save.' and stop.\n\n"
    "Do NOT capture as skills — these harden into constraints that outlive the "
    "problem:\n"
    "  • Environment-dependent failures: missing binaries, 'command not "
    "found', uninstalled packages, unconfigured credentials, fresh-install or "
    "post-migration path errors. Capture the FIX under an existing setup or "
    ...
    "If two existing skills overlap, mention it in your reply — the curator "
    "handles consolidation.\n\n"
    "If nothing clears the bar on either dimension, reply exactly "
    "'Nothing to save.'"
)
